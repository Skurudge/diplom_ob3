...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    # Выводим текстовое название статуса вместо кода (например, "Новый" вместо "NEW")
    status_display = serializers.CharField(source="get_status_display", read_only=True)
    # Показываем имя пользователя, который загрузил документ
    ...
    class Meta:
        ...
        # Эти поля пользователь не может менять вручную при отправке
...
class DocumentUploadSerializer(serializers.ModelSerializer):
    ...
    def validate_file(self, value: Any) -> Any:
        ...
        # Ограничим размер файла до 10 МБ, чтобы не перегружать сервер
